...
standard `Retry-After` header (seconds). Production enforces `1.1.1` as the minimum for this
diagnostic-run-aware release even when `MIN_CLIENT_VERSION` is blank, and honors any higher configured
minimum. Product requests whose `X-Client-Version` is missing, malformed, or lower are rejected with
`426 { code: 'CLIENT_UPGRADE_REQUIRED' }`; operational probes and explicit privacy/account exits remain
available. Response contracts are otherwise additive-only.
