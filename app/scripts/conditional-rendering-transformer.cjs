...
const { conditionalRenderingInstrumentationPlugin } = require('./conditional-rendering-core.cjs');

const projectRoot = path.resolve(
  process.env.CONDITIONAL_RENDER_PROJECT_ROOT || path.join(__dirname, '..'),
);

// Derive the options through jest-expo's own versioned resolver instead of
// copying a preset approximation. The sole addition is this campaign's Babel
// plugin; every root, caller, preset, and babelrc option stays identical to the
// installed jest-expo version.
const expoBabelOptions = resolveBabelOptions(projectRoot);
const delegate = babelJest.createTransformer({
  ...expoBabelOptions,
  plugins: [
    ...(expoBabelOptions.plugins || []),
    [conditionalRenderingInstrumentationPlugin, { projectRoot }],
  ],
});
...
const transformerSourceHash = createHash('sha256')
  ...
  .update('\0')
  .update(fs.readFileSync(path.join(__dirname, 'conditional-rendering-core.cjs')))
  .digest('hex');
...
module.exports = {
  ...
  getCacheKey(sourceText, sourcePath, transformOptions) {
    const upstreamKey = delegate.getCacheKey(sourceText, sourcePath, transformOptions);
