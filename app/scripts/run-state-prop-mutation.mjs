...
import {
  expectedTimeoutSiteIds,
  expectedTimeoutSites,
  reviewedStatusFor,
  validateExpectedTimeoutPins,
} from './state-prop-expectations.mjs';
...
const jestConfigPath = path.join(scriptsDirectory, 'state-prop-mutation.jest.config.cjs');
const reportSchemaVersion = 1;
// Each mutant runs a single-core `jest --runInBand` subprocess; 12 keeps a
// healthy host with idle cores saturated without starving Jest children.
const maximumConcurrency = 12;
...
const campaignToolInputFiles = Object.freeze([
  ...
  'scripts/run-state-prop-mutation.mjs',
  'scripts/state-prop-mutation-core.cjs',
  'scripts/state-prop-mutation-transformer.cjs',
  'scripts/state-prop-mutation.jest.config.cjs',
  'scripts/test-state-prop-mutation.mjs',
  'tsconfig.json',
]);
...
function defaultReportDirectory(appDir, mode) {
  return path.join(
    ...
    'reports',
    'mutation',
    mode === 'state' ? 'state-wiring' : 'prop-wiring',
  );
}

function siteKindsFor(mode) {
  return mode === 'state' ? STATE_SITE_KINDS : PROP_SITE_KINDS;
}

const campaignResultStatuses = Object.freeze([
  ...mutationResultStatuses,
  'ExpectedTimeout',
  'ExpectedError',
  'ExpectedEquivalent',
...
function countCampaignStatuses(results) {
  const counts = {
    Killed: 0,
    Survived: 0,
    Error: 0,
  ...
  for (const result of results) {
    if (!campaignResultStatuses.includes(result.status)) {
...
export function parseCliArgs(
  ...
  const sites = [];
  const kinds = [];
  let mode = environment.STATE_PROP_CAMPAIGN_MODE;
  let concurrency = environment.STATE_PROP_MUTATION_CONCURRENCY;
  let deadlineMs = environment.STATE_PROP_MUTATION_DEADLINE_MS;
  let reportDirectory = environment.STATE_PROP_MUTATION_REPORT_DIR;
  let list = false;
  let help = false;
...
export function usage() {
  return `Usage: node scripts/run-state-prop-mutation.mjs --mode MODE [options]

Modes:
  state   No-op every authored useState setter call and force every initial
          state value to a hostile same-family default.
  prop    Force every authored non-event, non-accessibility JSX prop to a
          hostile value (string/number/boolean/shorthand) or remove it
          (expression values).

Options:
  --lane NAME          Select a mutation lane (repeatable)
  --file APP_PATH      Select a mutable source file (repeatable)
  --site SITE_ID       Select an exact discovered site (repeatable)
  --kind KIND          Select a site kind (repeatable; state: ${STATE_SITE_KINDS.join(', ')}; prop: ${PROP_SITE_KINDS.join(', ')})
...
function sourceOwnership(lanes, mode) {
  const ownership = new Map();
  const extensionAllowed = (sourceFile) =>
    mode === 'state' ? /\.(ts|tsx)$/u.test(sourceFile) : sourceFile.endsWith('.tsx');
  for (const [laneName, lane] of Object.entries(lanes)) {
    for (const sourceFile of lane.mutate) {
      if (!extensionAllowed(sourceFile)) continue;
...
export async function discoverCampaignSites({
  ...
  const ownership = sourceOwnership(lanes, normalizedMode);
  const discovered = [];
  for (const sourceFile of [...ownership.keys()].toSorted()) {
    ...
    const sites = discoverStatePropMutationSites(source, {
  ...
  discovered.sort(
    (left, right) =>
      ...
      left.siteLocation.start.offset - right.siteLocation.start.offset,
  );
  const ids = discovered.map(({ id }) => id);
  if (new Set(ids).size !== ids.length)
    throw new Error('State/prop mutation site IDs are not unique');
  return discovered;
}
...
export function selectCampaignSites(allSites, filters, lanes = mutationLanes) {
  ...
  const selected = allSites.filter(
    (site) =>
      ...
      (kindFilter.size === 0 || kindFilter.has(site.kind)),
  );
  if (selected.length === 0) throw new Error('The requested filters intersect to zero sites');
  return selected;
}

function productionFilesFor(lanes, mode) {
  const ownership = sourceOwnership(lanes, mode);
  return uniqueSorted([...ownership.keys()]);
}
...
export async function executeStatePropMutationJest({
  appDir,
  mode,
  siteId = null,
  testFiles,
  ...
  return executeJestMutationProcess({
    ...
    // A state-wiring mutant can abort a test that had an assessment POST in
    // flight; its 150s real-timer abort deadlines then hold the Jest process
    // alive long after the report with every verdict is written. The mutant's
    // status comes only from that report, so end the post-verdict drain.
    forceExit: true,
  });
}
...
export async function runOneMutation({
  ...
  const expectedStatus = reviewedStatusFor(site.id, expectedTimeouts);
  const detected =
    (classification.evidenceFailureCount ?? 0) >= 1 ||
    (classification.failedTestNames?.length ?? 0) >= 1;
  const equivalentSurvivor = expectedStatus === 'Survived' && classification.status === 'Survived';
  if (
    (expectedStatus !== null &&
    ...
    return {
      ...
      siteId: site.id,
      status: mappedStatus,
      reason: `Detected by ${classification.evidenceFailureCount ?? classification.failedTestNames?.length ?? 0} failing assertions; the ${expectedStatus} classification is a reviewed harness limitation (${expectedTimeouts.find((entry) => entry.id === site.id)?.reason ?? ''})`,
...
export function assertCampaignReport(report) {
  ...
  if (report.campaign !== `state-prop:${report.mode}`) {
  ...
  const allowedKinds = siteKindsFor(report.mode);
  ...
  if (report.sites.some(({ kind }) => !allowedKinds.includes(kind))) {
    throw new Error(`Campaign report has an invalid ${report.mode} site kind`);
  }
  ...
  if (
    ...
    report.results.some(
      (result) =>
        ...
        !campaignResultStatuses.includes(result.status),
    )
  ) {
  ...
  if (
    JSON.stringify(countCampaignStatuses(report.results)) !==
  ...
  const strictPass =
    ...
    report.results.every(
      ({ status }) =>
        status === 'Killed' || status === 'ExpectedTimeout' || status === 'ExpectedError',
    ) &&
    report.provenance.inputsUnchanged;
  if (report.passed !== strictPass) {
    throw new Error(
      'Campaign report passed flag violates the strict killed-or-reviewed-timeout policy',
    );
  }
  return report;
}
...
export function renderCampaignMarkdown(report) {
  ...
  const title =
    report.mode === 'state'
      ? 'State Wiring Mutant Campaign'
      : 'Component Prop Wiring Mutant Campaign';
  const lines = [
    `# ${title}`,
    '',
    ...
    '',
    report.mode === 'state'
      ? 'Every authored useState setter call is disconnected with a no-op and every initial state value is forced to a hostile same-family default, including value kinds the general Stryker campaign already mutates; every site must be killed by an owning assertion.'
      : 'Every authored non-event, non-accessibility JSX prop is forced to a hostile value (string/number/boolean/shorthand) or removed (expression values), including value kinds the general Stryker campaign already mutates; every site must be killed by an owning assertion.',
    '',
    '## Input provenance',
    '',
    ...
    '## Sites and results',
    '',
    '| Site | Location | Lane | Kind | Original | Mutation | Result |',
    '| --- | --- | --- | --- | --- | --- | --- |',
  ];
  const resultById = new Map(report.results.map((result) => [result.siteId, result]));
  for (const site of report.sites) {
    const location = `${site.file}:${site.siteLocation.start.line}:${site.siteLocation.start.column}`;
    lines.push(
      `| ${markdownCell(site.id)} | ${markdownCell(location)} | ${markdownCell(site.laneName)} | ${markdownCell(site.kind)} | \`${markdownCell(site.siteSource)}\` | ${markdownCell(site.mutation)} | ${resultById.get(site.id)?.status || 'Not run'} |`,
    );
  }
...
function reportFileStem(mode) {
  return mode === 'state' ? 'state-wiring' : 'prop-wiring';
}
...
export async function runStatePropMutationCampaign({
  appDir = defaultAppDirectory,
  mode,
  reportDir,
  filters = { lanes: [], files: [], sites: [], kinds: [] },
  concurrency = 2,
  deadlineMs = 120_000,
  environment = process.env,
  validateManifest = assertMutationLaneManifest,
  lanes = mutationLanes,
  executeJest = executeStatePropMutationJest,
  log = console.log,
} = {}) {
  const normalizedMode = normalizeMode(mode);
  const resolvedReportDir = reportDir || defaultReportDirectory(appDir, normalizedMode);
  const campaign = `state-prop:${normalizedMode}`;
  ...
  try {
    ...
    // Pins are mode-scoped: a state-mode pin cannot match prop discovery and
    // vice versa, so only validate this mode's entries against it.
    const modePrefix = `sp:${normalizedMode}:`;
    const pinProblems = validateExpectedTimeoutPins(
      allSites,
      expectedTimeoutSites.filter(({ id }) => id.startsWith(modePrefix)),
    );
    if (pinProblems.length > 0) {
      throw new Error(
        [
          'State/prop expected-timeout pins are stale:',
          ...pinProblems.map((line) => `  - ${line}`),
        ].join('\n'),
      );
    }
    ...
    const snapshotOptions = {
      ...
      productionFiles: productionFilesFor(lanes, normalizedMode),
      testFiles,
    ...
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), `sp-${normalizedMode}-mutants-`),
    );
    let baseline;
    let results = [];
    let after = null;
    ...
    const statuses = countCampaignStatuses(results);
    const expectedMutationCount = sites.length;
    const acceptable =
      statuses.Killed + statuses.ExpectedTimeout + statuses.ExpectedError === expectedMutationCount;
    const passed =
    ...
    const report = {
      ...
      expectedTimeouts: expectedTimeoutSites.filter((entry) => siteIds.includes(entry.id)),
...
async function main() {
  ...
  if (options.list) {
    ...
    for (const site of sites) {
      console.log(
        `${site.id}\t${site.laneName}\t${site.kind}\t${site.siteSource.replaceAll(/\s+/gu, ' ')}`,
      );
    }
    console.log(`${sites.length} selected ${options.mode} state/prop mutation sites`);
    return;
  }

  const { report, reportPaths } = await runStatePropMutationCampaign(options);
  console.log(
    `${options.mode} state/prop campaign ${report.passed ? 'passed' : 'failed'}: ` +
