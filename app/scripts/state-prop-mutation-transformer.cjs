...
} = require('./state-prop-mutation-core.cjs');
...
const delegate = babelJest.createTransformer({
  ...
  plugins: [
    ...
    [statePropMutationInstrumentationPlugin, { projectRoot, mode }],
  ],
});
...
const transformerSourceHash = createHash('sha256')
  ...
  .update('\0')
  .update(fs.readFileSync(path.join(__dirname, 'state-prop-mutation-core.cjs')))
  .update('\0')
  .update(mode)
  .digest('hex');
