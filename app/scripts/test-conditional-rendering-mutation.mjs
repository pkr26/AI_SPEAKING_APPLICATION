...
test('the checked-in campaign discovers the expected 104 sites and 208 forced mutants', async () => {
  const sites = await discoverCampaignSites({ appDir: appDirectory });
  assert.equal(sites.length, 104);
  assert.equal(sites.length * 2, 208);
