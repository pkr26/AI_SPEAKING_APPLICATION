...
function hasUnsafeTextCharacter(value: string, allowNativeJoiners = false): boolean {
  const unsafeTextCharacter =
    // eslint-disable-next-line no-control-regex -- the validator intentionally rejects literal C0/C1 controls
    /[\u0000-\u001f\u007f-\u009f\u00ad\u061c\u200b\u200e\u200f\u2028-\u202e\u2060-\u206f\ufeff]/u;
  const defaultIgnorableCharacter = /\p{Default_Ignorable_Code_Point}/u;
  if (unsafeTextCharacter.test(value)) return true;

  for (const character of value) {
    if (!defaultIgnorableCharacter.test(character)) continue;
    const codePoint = character.codePointAt(0);
    if (!allowNativeJoiners || (codePoint !== 0x200c && codePoint !== 0x200d)) return true;
  }
  return false;
}
...
function canonicalCatalogText(value: string): string {
  // Upper-casing supplies the useful subset of Unicode case folding that JS
  // exposes directly (for example, both "straße" and "STRASSE" become
  // "STRASSE") while NFKC removes compatibility-presentation differences.
  // Case conversion can itself emit a decomposed sequence (for example,
  // U+0130 versus `i` + U+0307), so normalize again before comparing keys.
  return value.normalize('NFKC').toUpperCase().normalize('NFKC').replace(/\s+/gu, ' ');
}
...
export function renderSeedSql(seedQuestions: readonly QuestionSeed[] = questions): string {
  ...
  for (const question of seedQuestions) {
    ...
    const normalizedPromptWord = canonicalCatalogText(question.promptWord);
    ...
    const normalizedQuestionText = canonicalCatalogText(question.questionText);
    ...
    for (const lang of ['te', 'hi', 'es', 'zh'] as const) {
      ...
      for (const example of translation.examples) {
        ...
        if (example.native === example.en) {
          throw new Error(`untranslated example in ${lang} for "${question.promptWord}"`);
        }
        if (!hasRequiredNativeScript(lang, example.native)) {
          throw new Error(`empty example in ${lang} for "${question.promptWord}"`);
        }
      }
...
export function writeSeedSql(
  ...
  fs.writeFileSync(outputPath, renderSeedSql(seedQuestions));
